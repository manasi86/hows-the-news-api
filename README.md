...
genuine news content, summarizes it, and analyzes the sentiment of the summary
(positive, negative, or neutral). All heavy lifting is delegated to a
FreeLLM-compatible chat completions API.

- `src/api` exposes three endpoints: `POST /summarize`, `POST /analyse`, and
  `GET /health`.
- `src/lib` contains all business logic: the LLM client, the guardrailed
  prompts, news detection/summarization, and sentiment analysis.

# Configuration

Copy the example environment file and set your FreeLLM API key:

```shell
cp .env.example .env
```

The following environment variables are read from `.env` (or the environment):

| Variable        | Description                                            | Default                     |
|-----------------|--------------------------------------------------------|-----------------------------|
| `LLM_API_KEY`   | FreeLLM API key (required)                             | —                           |
| `LLM_BASE_URL`  | FreeLLM OpenAI-compatible base URL                     | `https://llm.nalits.com/v1` |
| `LLM_MODEL`     | Model name sent in chat completion requests            | `auto`                     |
| `LLM_TIMEOUT`   | HTTP timeout in seconds for LLM requests               | `30.0`                      |
...
Checks whether the text is genuine news content and, if so, summarizes it.

```shell
curl -X POST http://127.0.0.1:8000/summarize \
  -H "Content-Type: application/json" \
  -d '{"text": "The city council voted to fund the new public library."}'
```

## Analyse

Analyzes the sentiment of a text summary as `positive`, `negative`, or `neutral`.
...
This will generate `dist/hows_the_news-1.0.0.tar.gz` and `dist/hows_the_news-1.0.0-py3-none-any.whl`.
...
can be viewed at <https://nalits.github.io/hows-the-news/>.
...
```python
"""Check whether ``text`` is news content and, if so, summarize it.
...
1. Calling `python` in the project root (for example, `python -m pytest tests/`)
   ...
   packages are in the project root, they shadow the packages installed in the Nox
   session.
...
The `api` package exposes the HTTP endpoints and Pydantic schemas. All business
logic — including the FreeLLM client and the guardrailed prompts — lives in the
`lib` package.
...
```shell
docker build --tag hows-the-news .
```

To run the image in a container, passing the FreeLLM API key at runtime:

```shell
docker run --rm -e LLM_API_KEY=your-key-here -p 8000:8000 hows-the-news
```
...
This project is based on the [`python-blueprint`](https://github.com/johnthagen/python-blueprint)
template:
